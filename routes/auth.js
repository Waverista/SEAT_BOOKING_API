...
const validateInput = (fields) => {
  ...
  if (!fields.email || !/\S+@\S+\.\S+/.test(fields.email)) errors.email = 'Valid email is required';
  if (!fields.password || fields.password.length < 6) errors.password = 'Password must be at least 6 characters long';
...
router.post('/register', async (req, res) => {
  ...
  if (Object.keys(errors).length > 0) return res.status(400).json({ message: 'Validation errors', errors });
